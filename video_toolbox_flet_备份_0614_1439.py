"""
电商工具箱 v3.2 — Flet 版本
Tab 1: 视频去重 | Tab 2: 无水印下载 | Tab 3: 语音转文案 | Tab 4: AI生图 | 设置
"""
import os, sys, random, subprocess, re, time, threading, json, hashlib
...
def main(page: ft.Page):
    ...
    settings_status = ft.Text('', size=12, color=TEXT_MUTED)

    def save_all_settings(e):
        cfg = {}
        if os.path.exists(_cfg_file):
            try:
                with open(_cfg_file) as f: cfg = json.load(f)
            except: pass
        cfg['minimax_api_key'] = api_key_field.value
        cfg['img_save_path'] = img_path_field.value
        with open(_cfg_file, 'w') as f: json.dump(cfg, f)
        settings_status.value = '✅ 已保存'
        settings_status.color = GREEN
        safe_update()

    def _card(title, desc, body=None):
        return ft.Container(
            content=ft.Column([
                ft.Row([ft.Container(width=3, height=18, bgcolor=ACCENT if body else TEXT_MUTED, border_radius=2),
                    ft.Text(title, size=15, weight='bold', color=ACCENT if body else TEXT_MUTED)], spacing=8),
                ft.Text(desc, size=12, color=TEXT_MUTED),
                *([] if not body else [ft.Divider(height=1, color=BORDER)] + body),
            ], spacing=10),
            bgcolor=BG_PANEL, border_radius=12, padding=20, border=_bd(),
        )

    tab5 = ft.Column([
        ft.Text('设置', size=18, weight=ft.FontWeight.W_600, color=TEXT_PRIMARY),
        ft.Divider(height=1, color=BORDER),
        _card('🎨 AI生图', 'MiniMax image-01 API 配置', [
            ft.Text('🔑 API Key', size=13, weight='bold', color=TEXT_PRIMARY),
            ft.Row([api_key_field, ft.Text('已配置' if _saved_key else '未配置', size=12, color=GREEN if _saved_key else RED)], spacing=10),
            ft.Text('📁 保存路径', size=13, weight='bold', color=TEXT_PRIMARY),
            ft.Row([img_path_field, ft.ElevatedButton('选择', icon=ft.Icons.FOLDER_OPEN,
                on_click=lambda e: img_dir_picker.get_directory_path(), height=34)], spacing=10),
        ]),
        _card('📹 视频去重', '暂无独立设置'),
        _card('📥 视频下载', '暂无独立设置'),
        _card('🎙 文案提取', '暂无独立设置'),
        ft.ElevatedButton('💾 保存设置', icon=ft.Icons.SAVE, bgcolor=ACCENT, color='white', on_click=save_all_settings, height=40),
        settings_status,
    ...
    def on_nav_change(e):
        ...
        content_area.content = [tab1, tab2, tab3, tab4, tab5][idx]
        safe_update()
...
def auth_page(page: ft.Page):
    ...
    action_btn = ft.ElevatedButton("登 录", width=320, height=44,
    ...
    def toggle(e):
        ...
        title_text.value = "登录" if is_login[0] else "注册"
        confirm_pw.visible = not is_login[0]
        action_btn.text = "登 录" if is_login[0] else "注 册"
    ...
    toggle_btn = ft.TextButton("没有账号？去注册", on_click=toggle)
