...
def main(page: ft.Page):
    ...
    img_prompt = ft.TextField(
        ...
        multiline=True, min_lines=12, max_lines=12,
        bgcolor=BG_SURFACE, color=TEXT_PRIMARY,
        border_color=BORDER, border_radius=8,
        text_size=13, content_padding=ft.Padding(left=12, top=12, right=12, bottom=12),
        hint_style=ft.TextStyle(color="#62666D"),
    ...
    opt_result_text = ft.TextField(
        ...
        multiline=True, min_lines=12, max_lines=12, read_only=True,
        bgcolor=BG_SURFACE, color=TEXT_PRIMARY,
        border_color=BORDER, border_radius=8,
        text_size=13, content_padding=ft.Padding(left=12, top=12, right=12, bottom=12),
        hint_style=ft.TextStyle(color="#62666D"),
    ...
    def optimize_prompt(e):
        ...
        def worker():
            try:
                ...
                body = json.dumps({
                    "model": "MiniMax-M2.7",
                    "messages": [{"role": "user", "content": f"你是图片提示词优化专家。请将以下提示词优化得更详细、更专业，用于AI生图。添加光线、构图、风格细节。只输出优化后的提示词，不要解释。\n\n原文：{original}"}],
    ...
    def generate_image(e):
        original = img_prompt.value.strip()
        prompt = optimized_prompt[0] if use_optimized[0] and optimized_prompt[0] else original
        if not prompt:
    ...
    tab4 = ft.Column([
        ...
        ft.Container(
            content=ft.Row([
                ...
                ft.ElevatedButton("用原始", on_click=select_original, height=34, bgcolor=BG_SURFACE),
                ft.ElevatedButton("用优化", on_click=select_optimized, height=34, bgcolor=GREEN, color="white"),
